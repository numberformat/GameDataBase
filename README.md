...
* Data normalization
* Structural integrity (relational forms)
* Reproducible monthly dataset releases

The normalized datasets are published as downloadable release artifacts.

---

## Project Structure

This repository maintains **two parallel concerns**:

### 1. Upstream Source (Raw / Bronze)

Unmodified CSV files from the original upstream project.
These are treated as the **authoritative source of truth**.

### 2. Normalized Output (Silver)

Generated by a reproducible pipeline:

```
raw CSV → 1NF → 3NF → validation → release artifact
```

The normalized files are **not committed to the repository**. They are built inside a container and published as versioned release assets.

---

## Data Releases

Each valid build publishes:

```
game_database_3nf.zip
```

Releases are versioned by date using tags:
...
```
https://github.com/<fork>/GameDataBase/releases/download/v2025.11.26/game_database_3nf.zip
```

The filename remains stable so automated consumers can rely on consistent naming.

---

## Normalization Strategy

The dataset is normalized to **Third Normal Form (3NF)**.
...
The gold/analytics layer is intentionally not published.

---

## Build Process

The system is fully automated via Docker and GitHub Actions:
...
2. Run normalization container
3. Run data quality checks
4. Package `{data/3nf}`
5. Publish as GitHub Release asset

Only artifacts are published — the repository remains code-only.

The remainder of this README file is directly from the upstream project.
...
| `:rpg` | Role-Playing Game | `>a`<br>`>j`<br>`>s`<br>`>dungeoncrawler`<br>`>roguelike`<br>`>mmo` | ARPG / Action RPG<br>JRPG<br>SRPG / Tactics RPG<br>Dungeon crawler<br>Roguelike<br>MMO / Massively Multiplayer Online |
...
| `:vr` | Virtual Reality |  |  |
| `:software` | Software tool, engine or somekind of middleware used | `>musyx` | Factor 5 MusyX |
...
| `:ebikore` | Enterbrain EBColle Price / エビコレプライズ |
